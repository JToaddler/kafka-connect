...
import static com.madlabs.kafka.gsc.GitHubSchemas.CREATED_AT_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.KEY_SCHEMA;
import static com.madlabs.kafka.gsc.GitHubSchemas.NEXT_PAGE_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.NUMBER_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.OWNER_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.PR_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.PR_HTML_URL_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.PR_SCHEMA;
import static com.madlabs.kafka.gsc.GitHubSchemas.PR_URL_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.REPOSITORY_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.STATE_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.TITLE_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.UPDATED_AT_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.URL_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.USER_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.USER_ID_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.USER_LOGIN_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.USER_SCHEMA;
import static com.madlabs.kafka.gsc.GitHubSchemas.USER_URL_FIELD;
import static com.madlabs.kafka.gsc.GitHubSchemas.VALUE_SCHEMA;
...
import com.madlabs.kafka.gsc.utils.DateUtils;
import com.madlabs.kafka.gscmodel.Issue;
import com.madlabs.kafka.gscmodel.PullRequest;
import com.madlabs.kafka.gscmodel.User;
...
public class GitHubSourceTask extends SourceTask {
	...
	private SourceRecord generateSourceRecord(Issue issue) {
		return new SourceRecord(sourcePartition(), sourceOffset(issue.getUpdatedAt()), config.getTopic(), null, // partition
																												// will
																												// be
																												// inferred
																												// by
																												// the
																												// framework
				KEY_SCHEMA, buildRecordKey(issue), VALUE_SCHEMA, buildRecordValue(issue),
	...
	private Struct buildRecordKey(Issue issue) {
		// Key Schema
		Struct key = new Struct(KEY_SCHEMA).put(OWNER_FIELD, config.getOwnerConfig())
				.put(REPOSITORY_FIELD, config.getRepoConfig()).put(NUMBER_FIELD, issue.getNumber());
	...
	private Struct buildRecordValue(Issue issue) {
		...
		Struct valueStruct = new Struct(VALUE_SCHEMA).put(URL_FIELD, issue.getUrl()).put(TITLE_FIELD, issue.getTitle())
				.put(CREATED_AT_FIELD, issue.getCreatedAt().toEpochMilli())
				.put(UPDATED_AT_FIELD, issue.getUpdatedAt().toEpochMilli()).put(NUMBER_FIELD, issue.getNumber())
				.put(STATE_FIELD, issue.getState());

		// User is mandatory
		User user = issue.getUser();
		Struct userStruct = new Struct(USER_SCHEMA).put(USER_URL_FIELD, user.getUrl()).put(USER_ID_FIELD, user.getId())
		...
		if (pullRequest != null) {
			Struct prStruct = new Struct(PR_SCHEMA).put(PR_URL_FIELD, pullRequest.getUrl()).put(PR_HTML_URL_FIELD,
